...
export default function AboutUsRightForm({ fillHeight = false }: AboutUsRightFormProps) {
  ...
  return (
    <aside
      className={`${MontserratSans.className} w-full lg:max-w-none ${
        fillHeight ? "h-full" : ""
      }`}
    >
      <div
        className={`bg-[#f2f2f2] rounded-[18px] p-4 lg:p-5 ${
      ...
      >
        ...
        <form
          ...
          className={`space-y-3 ${fillHeight ? "flex-1 flex flex-col" : ""}`}
        >
          <input
            required
            type="text"
          ...
          <textarea
            ...
            onChange={(e) => setDataForm((p) => ({ ...p, Message: e.target.value }))}
          />
