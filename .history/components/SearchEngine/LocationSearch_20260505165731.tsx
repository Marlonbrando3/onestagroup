...
export default function LocationSearch({ value, onChange, className }: Props) {
  ...
  return (
    ...
    >
      <div className="border-yellow-500 bg-white rounded-[30px] h-full flex flex-col justify-center px-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {value.map((loc) => (
            <div
              key={loc.id}
              className="flex items-center gap-1 bg-yellow-300/[0.9] px-2 py-1 rounded-full text-sm"
            >
              <span>{loc.name}</span>
              <button
                onClick={() => removeLocation(loc.id)}
                className="text-gray-600 hover:text-gray-900 font-bold leading-none text-[22px]"
                aria-label="Remove"
              >
                ×
              </button>
      ...
      {open && results.length > 0 && (
        <div className="absolute mt-2 w-full bg-white border rounded shadow z-50 max-h-60 ">
