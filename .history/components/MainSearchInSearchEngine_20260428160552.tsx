  ...
  const removeFilter = (key: keyof FiltersState) => {
    ...
    setFilters((prev) => {
      const next: FiltersState = {
        ...prev,
        locations: key === "locations" ? [] : prev.locations,
        type: key === "type" ? [] : prev.type,
        bedrooms: key === "bedrooms" ? [] : prev.bedrooms,
        bathrooms: key === "bathrooms" ? [] : prev.bathrooms,
        price: key === "price" ? DEFAULT_PRICE : prev.price,
      };
      return next;
    });

    // 2. wyczyść URL (NA TWARDO)
    const q: any = { ...router.query };

    if (key === "locations") {
      delete q.location; // 🔥 kluczowe
      delete q.region; // 🔥 jeśli używasz region jako fallback
    }
    if (key === "type") delete q.type;
    if (key === "bedrooms") {
      delete q.beds;
      delete q.bedsMin;
      delete q.bedsMax;
