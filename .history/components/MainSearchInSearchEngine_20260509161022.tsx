...
export default function Home({
  ...
  const [mobileTriggerHeight, setMobileTriggerHeight] = useState(0);
  const [mobileTriggerWidth, setMobileTriggerWidth] = useState<number | null>(
    null,
  );
  const [mobileTriggerLeft, setMobileTriggerLeft] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const measureHeight = () => {
      if (mobileTriggerRef.current) {
        setMobileTriggerHeight(mobileTriggerRef.current.offsetHeight);
      }
    };

    const measureAnchorBox = () => {
      if (!mobileTriggerAnchorRef.current) return;
      const rect = mobileTriggerAnchorRef.current.getBoundingClientRect();
      setMobileTriggerWidth(rect.width);
      setMobileTriggerLeft(rect.left);
    };
    ...
    const onScroll = () => {
      ...
      const anchorTop =
        ...
        window.scrollY;
      setIsMobilePinned(window.scrollY + TOP_OFFSET >= anchorTop);
    };
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ height: isMobilePinned ? mobileTriggerHeight : "auto" }}
      >
        <button
          ref={mobileTriggerRef}
          onClick={() => setMobileModalOpen(true)}
          className={`w-full bg-white rounded-full shadow-lg p-4 flex border  border-yellow-600 items-center gap-3 text-gray-500 hover:shadow-xl transition-shadow ${
            isMobilePinned ? "fixed top-[10px] z-[9999]" : "relative"
          }`}
          style={
            isMobilePinned
              ? {
                  width: mobileTriggerWidth ?? undefined,
                  left: mobileTriggerLeft ?? undefined,
                  zIndex: 2147483647,
                }
              : undefined
          }
      ...
      <div
        id="search-wrapper"
        className={`${OutfitSans.className} mx-auto tracking-[1.2px] w-[90vw] max-w-[1330px] sticky lg:top-[100px] lg:z-30 mb-[30px] mt-0 lg:-mt-[70px]`}
      >
        {/* DESKTOP SEARCH BAR */}
