...
function AccentHeading({
  ...
  return (
    <div className={`flex items-start gap-[12px] ${className}`}>
      ...
      <h2 className="text-[25px] font-[700] leading-[1.12] tracking-[-0.3px] lg:text-[38px]">
        {children}
      </h2>
    </div>
  );
...
function OfferCard({ offer }: { offer: DesignProperty }) {
  const images = getOfferImages(offer.images).slice(0, 3);
  const [activeIndex, setActiveIndex] = useState(0);
  const activeImage = images[activeIndex] || "/bg_main_site_2.png";
  ...
  return (
    <div
      ...
      className="group overflow-hidden rounded-[24px] bg-[#f3e8df] shadow-[0_12px_34px_rgba(0,0,0,0.12)] outline-none transition-transform duration-300 hover:-translate-y-1 lg:rounded-[38px]"
    >
      ...
      <div className="px-[18px] pb-[18px] pt-[16px] lg:px-[24px] lg:pb-[24px] lg:pt-[20px]">
        ...
        <div className="mt-[6px] flex items-start justify-between gap-[12px]">
          <div>
            <h3 className="text-[22px] font-[800] leading-[1.1] lg:text-[28px]">
          ...
          <p className="shrink-0 text-right text-[22px] font-[900] leading-none text-[#f15b2a] lg:text-[28px]">
            {offer.price
              ? `${Number(offer.price).toLocaleString("pl-PL")} €`
              : "Cena na zapytanie"}
          </p>
        </div>
        ...
        <div className="mt-[18px] flex flex-wrap items-center gap-[8px]">
          ...
          <button
            ...
            className="h-[34px] rounded-full bg-black px-[14px] text-[12px] font-[800] text-white"
          >
            Więcej zdjęć
          </button>
        </div>
      ...
      >
        <div className="min-h-screen w-full overflow-hidden bg-[#ffc329]">
          <section className="relative min-h-[620px] bg-[#ffc329] px-[24px] lg:min-h-[760px] lg:px-[70px] lg:pb-[46px]">
            ...
            <div className="relative z-10 mx-auto grid h-full max-w-[1240px] grid-cols-1 gap-[26px] lg:grid-cols-[56%_44%]">
              <div>
          ...
          <section className="bg-[#f3e8df] px-[24px] py-[52px] lg:px-[70px] lg:py-[88px]">
            <div className="mx-auto max-w-[1240px]">
              <AccentHeading>
                Usługa zarządzania najmem <br></br>obejmuje m.in:
              </AccentHeading>
...
const offerRanges = [
  ...
  { min: 230_000, max: 270_000, limit: 2, includeMax: false },
...
> = async () => {
  ...
  for (const range of offerRanges) {
    let query = client
      .from("properties")
      .select(
        "external_id, price, town, province, images, type, headerAdvertisement, country",
      )
      .eq("new_build", true)
      .gte("price", range.min)
      .not("images", "is", null)
