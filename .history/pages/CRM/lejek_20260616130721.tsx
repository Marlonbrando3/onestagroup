...
import { crmUsers, isCrmAdmin, normalizeCrmEmail } from "@/components/crm/users";
...
export default function CRMPipelinePage() {
  ...
  const { contacts, error, addContact, updateContact, updateContactStatus, deleteContact, reload } = useCrmContacts(visibleOwner);
  ...
  const [activityPopupPosition, setActivityPopupPosition] = useState({ left: 120, top: 76 });
  ...
  const [addContactStage, setAddContactStage] = useState<CrmStatus | null>(null);
  const [pipelineContactForm, setPipelineContactForm] = useState(emptyPipelineContactForm);
  ...
  const columns = crmStatuses.map((status) => {
  ...
  const oldestActivityByContactId = useMemo(() => {
    ...
    activities.forEach((activity) => {
      ...
      grouped.set(activity.contactId, [...(grouped.get(activity.contactId) || []), activity]);
    });

    const entries = Array.from(grouped.entries()).map(([contactId, contactActivities]) => [
      contactId,
  ...
  useEffect(() => {
    ...
    async function loadActivities() {
      ...
      const query = visibleOwner ? `?owner=${encodeURIComponent(visibleOwner)}` : "";
  ...
  function handleTransferDrop(event: DragEvent<HTMLElement>) {
    ...
    if (contactId) {
      const nextOwner =
        crmUsers.find((user) => user.email !== contact?.pipelineOwner)?.email || crmUsers[0]?.email || "";
  ...
  function openPipelineContactPopup(status: CrmStatus) {
    setAddContactStage(status);
  ...
  async function addContactFromPipeline(event: FormEvent<HTMLFormElement>) {
    ...
    const contact = await addContact({
      ...
      pipelineOwner: visibleOwner === "all" ? currentUserEmail : visibleOwner,
  ...
  async function moveContactToPipeline() {
    ...
    const updated = await updateContact({ id: transferContactId, pipelineOwner: transferOwner });
    setIsTransferring(false);
  ...
  function openActivityDetailsPopup(event: MouseEvent<HTMLElement>, contactId: string) {
    setActivityPopupPosition(getActivityPopupPosition(event.currentTarget));
    setOpenActivityContactId((current) => (current === contactId && openActivityMode === "details" ? "" : contactId));
    setOpenActivityMode("details");
  }

  function toggleMissingActivityPopup(event: MouseEvent<HTMLElement>, contactId: string) {
    setActivityPopupPosition(getActivityPopupPosition(event.currentTarget));
    setActivityFormError("");
    setActivityForm({
      type: "Zadzwoń",
  ...
  async function addActivityFromPipeline(event: FormEvent<HTMLFormElement>, contactId: string) {
    event.preventDefault();
  ...
  async function toggleActivityStatus(activity: CrmActivity) {
    ...
    setActivities((current) =>
      current.map((item) => (item.id === activity.id ? { ...item, status: nextStatus } : item)),
    );
    ...
    const updatedActivities = activities.map((item) => (item.id === activity.id ? result.activity : item));
    setActivities(updatedActivities);
    if (nextStatus === "done") {
      const hasMorePlannedActivities = updatedActivities.some(
        (item) => item.contactId === activity.contactId && item.status === "planned",
      );
  ...
  return (
    <CrmLayout active="pipeline">
      <section className="crmPipelinePage">
        ...
        <div className="crmPipelineBoard">
          {columns.map((column) => {
            ...
            const probabilityLabel = probability === null ? "brak" : `${probability}%`;
            ...
            >
              <div
                className={`crmPipelineColumnHeader ${
                  column.status === "Zakup w perspektywie" ? "highlightProspect" : ""
                }`}
              >
              ...
              <div className="crmPipelineCards">
                {column.contacts.map((contact) => {
                  const activity = oldestActivityByContactId.get(contact.id);
                  const isActivityOpen = openActivityContactId === contact.id;
                        ...
                        <span className="crmContactInfoTrigger">
                          ...
                          <section className="crmContactInfoPopup">
                            <dl>
                              <div>
                                <dt>Kraj</dt>
                                <dd>{formatContactInfoValue(contact.country)}</dd>
                              </div>
                              <div>
                                <dt>Kwota max</dt>
                                <dd>
                                  {contact.maxBudget ? crmBudgetFormatter.format(contact.maxBudget) : "Brak"}
                                </dd>
                              </div>
                              <div>
                                <dt>Sypialnie</dt>
                                <dd>{formatContactInfoValue(contact.bedrooms)}</dd>
                              </div>
                              <div>
                                <dt>Łazienki</dt>
                                <dd>{formatContactInfoValue(contact.bathrooms)}</dd>
                              </div>
                              <div>
                                <dt>Wybrzeże</dt>
                                <dd>{formatContactInfoValue(contact.coast)}</dd>
                              </div>
                              <div>
                                <dt>Termin zakupu</dt>
                                <dd>{formatContactInfoValue(contact.purchaseTimeline)}</dd>
                              </div>
                            </dl>
                          </section>
                        </span>
                        ...
                        <div className="crmCardActivity">
                          ...
                          {isActivityOpen ? (
                            <section
                              ...
                              style={{ left: activityPopupPosition.left, top: activityPopupPosition.top }}
                                ...
                                <>
                                  <div className="crmActivityBannerTop">
                                    ...
                                    <div>
                                      ...
                                      <p>
                                        {getActivityLabel(activity)} · {formatActivityDate(activity)} · Onesta
                                      </p>
                                    </div>
                                  </div>
                                  {activity.note ? <small>{activity.note}</small> : null}
                                  <button type="button" onClick={() => openAddActivityPopup(contact.id)}>
                                    + Zaplanuj działanie
                                  </button>
                                </>
                              ) : (
                                <>
                                  <div className="crmActivityBannerTop">
                                    ...
                                    <div>
                                      ...
                                      <p>Dodaj kolejne zadanie bez otwierania rekordu</p>
                                    </div>
                                  </div>
                                  <div className="crmActivityTypeTabs" aria-label="Typ zadania">
                                  ...
                                  <form
                                    ...
                                    onSubmit={(event) => addActivityFromPipeline(event, contact.id)}
                                  >
                                    <input
                                    ...
                                    <div className="crmPipelineActivityDate">
                                      ...
                                      <input
                                        ...
                                        onChange={(event) =>
                                          setActivityForm({ ...activityForm, dueTime: event.target.value })
                                        }
                                      />
                                    </div>
                                    <textarea
                        ...
                        <div className="crmCardActivity">
                          ...
                          {isActivityOpen ? (
                            ...
                            >
                              ...
                              <div className="crmActivityBannerTop">
                                <span className="crmMissingTask static">!</span>
                                <div>
                                  <strong>Brak zaplanowanego zadania</strong>
                                  <p>Dodaj zadanie bez otwierania rekordu</p>
                                </div>
                              </div>
                              <div className="crmActivityTypeTabs" aria-label="Typ zadania">
                                {crmActivityTypes.map((type) => (
        ...
        {transferContactId ? (
          <section className="crmTransferPopup" aria-label="Przenies kontakt">
            <button aria-label="Zamknij przenoszenie" type="button" onClick={closeTransferPopup}>
              ×
            </button>
            <strong>Przenieś do lejka</strong>
            <select value={transferOwner} onChange={(event) => setTransferOwner(event.target.value)}>
              {crmUsers.map((user) => (
                <option key={user.email} value={user.email}>
                  {user.label}
                </option>
            ...
            <button disabled={isTransferring} type="button" onClick={moveContactToPipeline}>
              {isTransferring ? "Przenoszenie..." : "Przenieś"}
            </button>
          </section>
        ) : null}
        {addContactStage ? (
          <section className="crmPipelineContactModal" aria-label="Dodaj kontakt z lejka">
            <form className="crmPipelineContactForm" onSubmit={addContactFromPipeline}>
              <button aria-label="Zamknij dodawanie kontaktu" type="button" onClick={closePipelineContactPopup}>
                ×
              </button>
              <div>
                <p>Nowy kontakt</p>
              ...
              <select
                value={addContactStage}
                onChange={(event) => setAddContactStage(event.target.value as CrmStatus)}
              >
              ...
              <input
                ...
                onChange={(event) => setPipelineContactForm({ ...pipelineContactForm, name: event.target.value })}
              />
              <input
                placeholder="Email"
                type="email"
                value={pipelineContactForm.email}
                onChange={(event) => setPipelineContactForm({ ...pipelineContactForm, email: event.target.value })}
              />
              <input
                placeholder="Telefon"
                value={pipelineContactForm.phone}
                onChange={(event) => setPipelineContactForm({ ...pipelineContactForm, phone: event.target.value })}
              />
              <input
                inputMode="numeric"
                ...
                onChange={(event) =>
                  setPipelineContactForm({ ...pipelineContactForm, maxBudget: event.target.value })
                }
              />
              <input
                placeholder="Kraj"
                value={pipelineContactForm.country}
                onChange={(event) => setPipelineContactForm({ ...pipelineContactForm, country: event.target.value })}
              />
              <input
                placeholder="Sypialnie"
                value={pipelineContactForm.bedrooms}
                onChange={(event) => setPipelineContactForm({ ...pipelineContactForm, bedrooms: event.target.value })}
              />
              <input
                placeholder="Łazienki"
                value={pipelineContactForm.bathrooms}
                onChange={(event) => setPipelineContactForm({ ...pipelineContactForm, bathrooms: event.target.value })}
              />
              <input
                placeholder="Wybrzeże"
                value={pipelineContactForm.coast}
                onChange={(event) => setPipelineContactForm({ ...pipelineContactForm, coast: event.target.value })}
              />
              <input
                placeholder="Termin zakupu"
                value={pipelineContactForm.purchaseTimeline}
                onChange={(event) =>
                  setPipelineContactForm({ ...pipelineContactForm, purchaseTimeline: event.target.value })
                }
              />
              {pipelineContactFormError ? <p>{pipelineContactFormError}</p> : null}
      ...
      <style jsx>{`
        ...
        .crmPipelineContactForm > button:first-child {
          align-items: center;
          background: #f4f6f8;
          border: 1px solid #d8dee7;
          border-radius: 6px;
        ...
        .crmPipelineContactForm > div {
          display: grid;
          gap: 3px;
          padding-right: 36px;
        }

        .crmPipelineContactForm > div p {
          color: #216e63;
          font-size: 12px;
          font-weight: 900;
          letter-spacing: 0;
        ...
        .crmPipelineContactForm > div strong {
          font-size: 18px;
          line-height: 1.15;
        }
        ...
        .crmPipelineContactForm > button:last-child {
          background: #216e63;
          border: 0;
          border-radius: 8px;
          color: #ffffff;
        ...
        .crmPipelineContactForm > button:last-child:disabled {
          cursor: not-allowed;
          opacity: 0.72;
        }
        ...
        .crmPipelineColumnHeader.highlightProspect {
          ...
          box-shadow: inset 3px 0 0 #f59e0b, 0 1px 0 rgba(255, 255, 255, 0.75);
        }
