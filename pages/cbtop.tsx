...
const WHATSAPP_HREF =
  "https://wa.me/48576652525?text=Dzie%C5%84%20dobry%2C%20pisz%C4%99%20w%20sprawie%20ofert%20z%20prezentacji%20Onesta.";
...
function WhatsAppContactLink({
  ...
  return (
    <a
      ...
      className={`group mt-5 flex min-h-[48px] w-full items-center justify-center gap-2.5 rounded-full border px-5 text-[10px] font-extrabold uppercase tracking-[0.09em] transition ${
        dark
          ? "border-[#25D366] bg-[#25D366] text-white hover:border-[#25D366]/40 hover:bg-[#25D366]/10"
          : "border-[#25D366] bg-[#25D366] text-white hover:border-[#25D366]/35 hover:bg-[#f1fcf5] hover:text-[#182334]"
      }`}
    >
      <RiWhatsappFill
        className="h-6 w-6 shrink-0 text-white transition group-hover:text-[#25D366]"
        aria-hidden="true"
      />
  ...
  const submitContactForm = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      trackCbTopInteraction({
        ...
        offerId: String(offer.external_id),
      });
...
export default function CbTopPage({ offers, socialImage }: CbTopPageProps) {
  ...
  const [pagePrivacyAccepted, setPagePrivacyAccepted] = useState(false);
  ...
  const submitPageContactForm = async (
    ...
    event.preventDefault();
    if (!pagePrivacyAccepted || pageFormStatus === "sending") return;

    setPageFormStatus("sending");
    try {
      const response = await fetch("/api/formFromProperty", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id: "CBTOP",
          name: pageContactForm.fullName.trim(),
          mail: pageContactForm.email.trim(),
          phone: pageContactForm.phone.trim(),
          massege:
            "Proszę o kontakt w sprawie wyboru nieruchomości z prezentacji /cbtop.",
          ref: "Prezentacja /cbtop",
          consents: {
            rodo: pagePrivacyAccepted,
            marketing: false,
          },
        }),
      });
  ...
  return (
    <>
      ...
      >
        <section className="relative overflow-hidden bg-[#182334] text-white">
          ...
          <div className="relative mx-auto grid w-11/12 max-w-[1380px] gap-8 py-9 sm:py-11 lg:grid-cols-[minmax(0,1fr)_390px] lg:items-end lg:py-12">
            ...
            <div className="hidden overflow-hidden rounded-2xl border border-white/15 bg-white/[0.06] px-6 py-6 backdrop-blur-sm md:block sm:px-7">
        ...
        <main className="relative mx-auto w-11/12 max-w-[1380px] pb-20">
          ...
          <section className="relative mt-12 overflow-hidden rounded-[28px] bg-[#182334] px-7 py-10 text-white sm:px-11 sm:py-12 lg:px-14 lg:py-14">
            <div className="absolute -right-20 -top-24 h-64 w-64 rounded-full border border-[#d6b66f]/20" />
            <div className="absolute -bottom-36 left-[18%] h-72 w-72 rounded-full bg-[#445d78]/25 blur-3xl" />
            <div className="relative grid gap-9 lg:grid-cols-[minmax(0,0.82fr)_minmax(460px,1fr)] lg:items-center lg:gap-14">
              <div className="max-w-[620px]">
                ...
                <p className="mt-4 text-[12px] leading-6 text-slate-300">
                  ...
                  inwestycji. Zostaw kontakt — doradca Onesta pomoże Ci wybrać
                  oferty najlepiej dopasowane do Twojego planu.
                </p>
              </div>

              <div className="rounded-[24px] bg-white p-6 text-[#182334] shadow-[0_22px_60px_rgba(0,0,0,0.18)] sm:p-8">
                {pageFormStatus === "success" ? (
                  <div className="flex min-h-[330px] flex-col items-center justify-center text-center">
                    <span className="flex h-16 w-16 items-center justify-center rounded-full bg-[#efe4d0] text-[#9b7a36]">
                      <FiCheckCircle className="h-7 w-7" aria-hidden="true" />
                    </span>
                    <p className="mt-5 text-[8px] font-extrabold uppercase tracking-[0.16em] text-[#9b7a36]">
                      Wiadomość wysłana
                    </p>
                    <h3
                      className={`${HomePlayfairSans.className} mt-2 text-[31px] font-semibold leading-tight`}
                    ...
                    <p className="mt-3 max-w-sm text-[11px] leading-5 text-slate-500">
                      Doradca Onesta wkrótce skontaktuje się z Tobą i pomoże
                      porównać wybrane nieruchomości.
                    </p>
                  </div>
                ) : (
                  <>
                    ...
                    <h3
                      className={`${HomePlayfairSans.className} mt-2 text-[28px] font-semibold leading-tight`}
                    >
                    ...
                    <form
                      ...
                      onSubmit={submitPageContactForm}
                    >
                      <div className="grid gap-4 sm:grid-cols-2">
                        <label className="block">
                          <span className="mb-1.5 block text-[8px] font-extrabold uppercase tracking-[0.12em] text-slate-500">
                            Imię i nazwisko
                          </span>
                          <input
                            required
                            type="text"
                            autoComplete="name"
                            value={pageContactForm.fullName}
                            onChange={(event) => {
                              setPageContactForm((current) => ({
                                ...current,
                                fullName: event.target.value,
                              }));
                              if (pageFormStatus === "error")
                                setPageFormStatus("idle");
                            }}
                            placeholder="Jan Kowalski"
                            className="h-12 w-full rounded-xl border border-[#ddd3c4] bg-[#faf8f4] px-4 text-[12px] font-semibold text-[#182334] outline-none transition placeholder:text-slate-400 focus:border-[#b8954c] focus:bg-white"
                          />
                        </label>
                      ...
                      <label className="flex cursor-pointer items-start gap-3 text-[9px] leading-4 text-slate-500">
                        <input
                          required
                          type="checkbox"
                          checked={pagePrivacyAccepted}
                          onChange={(event) =>
                            setPagePrivacyAccepted(event.target.checked)
                          }
                        ...
                        <span>
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="font-bold text-[#9b7a36] underline underline-offset-2"
                          >
                            Polityką Prywatności
                          </a>
                          .
                      ...
                      {pageFormStatus === "error" ? (
                        <p
                          className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-[9px] font-semibold text-red-700"
                          role="alert"
                        >
                          Nie udało się wysłać formularza. Spróbuj ponownie za
                          chwilę.
                      ...
                      <button
                        type="submit"
                        disabled={pageFormStatus === "sending"}
                        className="flex min-h-[50px] w-full items-center justify-center gap-2 rounded-full bg-[#d6b66f] px-6 text-[10px] font-extrabold uppercase tracking-[0.11em] text-[#182334] transition hover:bg-[#182334] hover:text-white disabled:cursor-wait disabled:opacity-65"
                      >
                        {pageFormStatus === "sending" ? (
                          <>
                            <span className="cbtop-form-loader h-4 w-4 rounded-full border-2 border-[#182334]/20 border-t-[#182334]" />
                            Wysyłanie
                          </>
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        aria-label="Porozmawiajmy na WhatsApp"
        onClick={() =>
          trackCbTopInteraction({
            event: "whatsapp_click",
            context: "floating",
        ...
        className="group fixed bottom-4 right-4 z-50 flex h-14 w-14 items-center justify-center rounded-full border border-[#25D366]/35 bg-white text-[#182334] shadow-[0_14px_35px_rgba(24,35,52,0.22)] transition duration-200 hover:-translate-y-0.5 hover:border-[#25D366] hover:bg-[#25D366] hover:text-white sm:bottom-6 sm:right-6 sm:w-auto sm:gap-2.5 sm:rounded-full sm:px-4"
      >
        <RiWhatsappFill
          className="cbtop-whatsapp-attention h-8 w-8 shrink-0 text-[#25D366] transition group-hover:text-white"
          aria-hidden="true"
        />
        <span className="hidden text-[10px] font-extrabold uppercase tracking-[0.08em] sm:block">
          Porozmawiajmy na WhatsApp
        </span>
      </a>
      ...
      <style jsx global>{`
        ...
        @keyframes cbtop-whatsapp-attention {
          0%,
          78%,
          100% {
        ...
        .cbtop-whatsapp-attention {
          animation: cbtop-whatsapp-attention 4.5s ease-in-out infinite;
